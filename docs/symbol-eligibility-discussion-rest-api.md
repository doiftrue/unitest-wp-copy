...
**Статус:** решение принято и реализовано. Используется Вариант 2:
generated trait с выбранными методами `WP_REST_Server`, namespaced
`WP_REST_Server__Runtime` и guarded `class_alias()` для глобального имени
`WP_REST_Server`. Корневой index сохраняет метаданные сайта, namespaces,
routes и help-link, но намеренно не включает theme/logo/icon/media enrichment.
...
Рабочий прецедент — `\Unitest_WP_Copy\wpdb__Runtime`: семь методов `wpdb`
...
- `WP_HTTP_Response{}`, `WP_REST_Request{}`, `WP_REST_Response{}`,
  `WP_REST_Controller{}`, `WP_Error{}`
- `\Unitest_WP_Copy\WP_REST_Server__Runtime` и его глобальный alias
  `WP_REST_Server`
- `register_rest_route()`, `rest_get_server()`, `rest_do_request()`,
  `rest_ensure_request()`, `rest_ensure_response()`,
...
По-прежнему отсутствуют намеренно: live HTTP serving, REST bootstrap/rewrite
lifecycle, ядровые endpoint-контроллеры, cookie/application-password auth,
post/term route resolution и theme/media enrichment корневого index.


## Инвентаризация зависимостей (повторно проверено по исходникам WP 7.1)
...
| `rest_convert_error_to_response()` | `sanitize_params()`, `has_valid_params()` | доступен |
| `rest_parse_request_arg()` | default sanitization в `sanitize_params()` | уже был доступен |
| `rest_url()` | `from_url()` | уже был доступен |
| `apply_filters()` | порядок параметров и `from_url()` | уже был доступен |
...
| `get_data_for_routes` / `get_data_for_route` | `rest_url`; в WP 7.1 также `wp_get_json_schema_allowed_keywords()` |
...
**Обогащение индекса (намеренно адаптировано и исключено):**

| Метод | Отсутствующая зависимость |
| --- | --- |
| `get_index` | в WP 7.1 дополнительно требует `wp_is_client_side_media_processing_enabled()`, capability checks и image-size registry |
...
Это пять непосредственных HTTP primitives плюс `serve_request()`.
`send_headers()` безопасно копируется без изменений после адаптации вызываемого
им `send_header()`. Всё, что нужно для регистрации и
...
1. **Константы статусов `WP_Http`** — `WP_Http::BAD_REQUEST`,
   ...
   Решение: правила в `Source_Code_Replacer` заменяют
   `WP_Http::BAD_REQUEST` и `WP_Http::MULTI_STATUS` числовыми статусами. Это не
   публикует вводящий в заблуждение частичный класс `WP_Http`.
...
3. **`current_user_can()`** — достижима только из обогащения индекса. Обработка
   Решение: кастомный мок с дефолтом `false` и переопределением через
   `WP_Mock`; неподдерживаемые index-enrichment методы не входят в адаптер.
...
4. **Опции, которые надо добавить в `$GLOBALS['stub_wp_options']`**:
   ...
   `site_icon => 0`. 
   Значения добавлены в глобальный массив опций.
...
5. **Сброс состояния между тестами** — сервер REST хранится в глобальной
   ...
   Рецепт добавлен в `README.md`, `docs/runtime.md` и `docs/tests.md`.
...
```php
// wp-runtime/custom-mocks/wp-includes/class-wp-rest-server.php
namespace Unitest_WP_Copy;

class WP_REST_Server__Runtime extends \WP_REST_Server {
...
и подключаем через отдельный runtime boot-файл:

```php
// wp-runtime/boot-wp-hooks.php
add_filter( 'wp_rest_server_class', static fn() => \Unitest_WP_Copy\WP_REST_Server__Runtime::class );
```
...
- Используются только те механизмы, что уже есть в проекте (`custom-mocks`,
  runtime boot-файл, ядровой фильтр `wp_rest_server_class`).
...
## Вариант 2 — Трейт instance-методов + runtime-adapted сервер (ВЫБРАН И РЕАЛИЗОВАН)

**Идея.** Точно повторить существующий паттерн `wpdb` / `wpdb__Runtime`.
...
`\Unitest_WP_Copy\WP_REST_Server__Runtime` в `wp-runtime/custom-mocks/`. Класс
...
Чтобы новый класс следовал уже реализованной модели `wpdb__Runtime`:
...
Реализован **Вариант 2: generated instance-method trait + документируемый
runtime-adapted класс**. После появления
...
1. `WP_REST_Request` и `WP_REST_Controller` копируются как полные классы.
2. Безопасные методы `WP_REST_Server` генерируются в
   `WP_REST_Server__Copied_Methods`; глобальные class dependencies импортируются
   в namespaced trait через `imports` в `config/instance-methods.php`.
3. `\Unitest_WP_Copy\WP_REST_Server__Runtime` предоставляет свойства и константы,
   адаптирует live HTTP boundary и корневой index.
4. Guarded `class_alias()` публикует адаптер как `WP_REST_Server`.
   `rest_get_server()` остаётся исходным WordPress-кодом, поэтому стандартный
   `wp_rest_server_class` filter продолжает работать; отдельный постоянный
   filter не устанавливается.
5. `is_user_logged_in()` и `current_user_can()` — постоянные custom mocks с
   дефолтом `false` и поддержкой `WP_Mock`.
6. `WP_Http` не объявляется частичным классом: две нужные константы заменяются
   parser compatibility rules.
7. Состояние REST сбрасывается обычным
   `unset( $GLOBALS['wp_rest_server'] )`; отдельный базовый TestCase не нужен.

Генерируемый трейт вручную не меняется и не считается публичным символом;
ожидаемый внешний контракт проверяется по сгенерированной секции
`WP_REST_Server__Runtime` в `SYMBOLS-INFO.md`.
